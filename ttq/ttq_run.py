...
class Model(ModelDesc):
    def __init__(self, bita, net_fn, quant_ends, tw_thres=0.05):
        super(Model, self).__init__()
        
        self.n_spks = 255
        self.n_context = 50
        self.tw_thres = tw_thres
        self.net_fn = net_fn
...
def create_dataflow(partition, cachedir, datadir, spkmap, sentfilt, context=50):
    ...
    if isTrain:
        ...
        rsr_ds = PrefetchDataZMQ(rsr_ds, min(6, multiprocessing.cpu_count()))
        logger.info("Using {} threads".format(min(6, multiprocessing.cpu_count())))
    else:
...
if __name__ == '__main__':
    ...
    out_dir = '_'.join([args.model_name, 'a', str(args.bita),'quant_ends', str(args.quant_ends)])
    if args.load_ckpt:
       out_dir += '_preload' 
    ...
    model = Model(args.bita, net_fn_map[args.model_name], args.quant_ends)
