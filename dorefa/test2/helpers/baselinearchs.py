...
def fcn1_net(inp, nl, context=50):
    ...
    with argscope(FullyConnectedWithTrackedMults, out_dim=256, nl=nl):
...
def fcn2_net(inp, nl, context=50):
    # https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/44681.pdf
    # sized so that total number of weights is 400000 is 230
    with argscope(FullyConnectedWithTrackedMults, out_dim=504, nl=nl):
        l = FullyConnectedWithTrackedMults('linear0', inp)
        l = FullyConnectedWithTrackedMults('linear1', l)
        l = FullyConnectedWithTrackedMults('linear2', l)
        l = FullyConnectedWithTrackedMults('linear3', l)
...
def maxout1_net(inp, nl, context=50):
    ...
    l = FullyConnectedWithTrackedMults('linear1',l,nl=nl, out_dim=1000)
    l = Maxout(l, 4)
    l = Dropout(l, keep_prob=0.4)
    l = FullyConnectedWithTrackedMults('linear2',l,nl=nl, out_dim=1000)
    l = Maxout(l, 4)
    l = FullyConnectedWithTrackedMults('linear3',l,nl=nl, out_dim=1000)
    l = Maxout(l, 4)
    return l

def maxout2_net(inp, nl, context=50):
    # https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/41939.pdf
    l = FullyConnectedWithTrackedMults('linear0',inp,nl=nl, out_dim=1000)
    l = Maxout(l, 2)
    l = Dropout(l, keep_prob=0.4)
...
def lcn_net(inp, nl, context=50):
    # https://pdfs.semanticscholar.org/ef8d/6c4c65a9a227f63f857fcb789db4202f2180.pdf
    l = tf.reshape(inp, (-1, context, 20, 1))
    with argscope(Conv2DWithTrackedMults, out_channel=25, kernel_shape=10, nl=nl, padding='valid'):
...
def dsc1_net(inp, nl, context=50):
    l = tf.reshape(inp, (-1, context, 20, 1))
    l = DepthwiseSeparableConvWithTrackedMults('conv0', l, 16, nl=nl)
    with argscope(FullyConnectedWithTrackedMults, out_dim=256, nl=nl):
        l = FullyConnectedWithTrackedMults('linear0', l)
        l = FullyConnectedWithTrackedMults('linear1', l)
        l = FullyConnectedWithTrackedMults('linear2', l)
...
def dsc2_net(inp, nl, context=50):
    l = tf.reshape(inp, (-1, context, 20, 1))
    with argscope(DepthwiseSeparableConvWithTrackedMults, nl=nl):
