...
def fcn_selu(inp, bn=False, context=50):
    ...
    with argscope(FullyConnectedWithTrackedMults, out_dim=256):
        l = FullyConnectedWithTrackedMults('linear0', inp, nl=tf.nn.selu)
        l = FullyConnectedWithTrackedMults('linear1', l, nl=tf.nn.selu)
        l = FullyConnectedWithTrackedMults('linear2', l, nl=tf.nn.selu)
        l = FullyConnectedWithTrackedMults('linear3', l, nl=tf.nn.selu)
    return l

def fcn_debug(inp, bn=True, context=50):
    # https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/44681.pdf
    # sized so that total number of weights is 400000 is 230
    if bn:
        with argscope(FullyConnectedWithTrackedMults, out_dim=256):
            l = tf.nn.tanh(BatchNorm(FullyConnectedWithTrackedMults('linear0', inp)))
            l = tf.nn.tanh(BatchNorm(FullyConnectedWithTrackedMults('linear1', l)))
            l = tf.nn.tanh(BatchNorm(FullyConnectedWithTrackedMults('linear2', l)))
            l = tf.nn.tanh(BatchNorm(FullyConnectedWithTrackedMults('linear3', l)))
    else:
        with argscope(FullyConnectedWithTrackedMults, out_dim=256, nl=tf.nn.tanh):
...
def fcn_tanh(inp, bn=True, context=50):
    # https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/44681.pdf
    # sized so that total number of weights is 400000 is 230
    with argscope(FullyConnectedWithTrackedMults, out_dim=256):
        l = FullyConnectedWithTrackedMults('linear0', inp, nl=BNReLU)
        l = FullyConnectedWithTrackedMults('linear1', l, nl=BNReLU)
        l = FullyConnectedWithTrackedMults('linear2', l, nl=BNReLU)
        l = FullyConnectedWithTrackedMults('linear3', l, nl=BNReLU)
    return l
...
def maxout1_net(inp, bn=True, context=50):
    ...
    l = FullyConnectedWithTrackedMults('linear0', inp, out_dim=1000)
    if bn:
        l = BatchNorm('bn0', l)
    l = Maxout(l, 4)
    l = Dropout(l, keep_prob=0.4)
    l = FullyConnectedWithTrackedMults('linear1', l, out_dim=1000)
    if bn:
        l = BatchNorm('bn1', l)
    l = Maxout(l, 4)
    l = Dropout(l, keep_prob=0.4)
    l = FullyConnectedWithTrackedMults('linear2', l, out_dim=1000)
    if bn:
        l = BatchNorm('bn2', l)
    l = Maxout(l, 4)
    l = FullyConnectedWithTrackedMults('linear3', l, out_dim=1000)
    if bn:
        l = BatchNorm('bn3', l)
    l = Maxout(l, 4)
    return l
