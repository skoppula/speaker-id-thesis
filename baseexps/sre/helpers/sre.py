...
__all__ = ['WholeUtteranceAsFrameRsr2015', 'RandomFramesBatchRsr2015', 'WholeUtteranceAsBatchRsr2015', 'SreMfccFiles', 'RandomFramesBatchFromCacheSre', 'get_n_spks']
...
class SreMfccFiles(RNGDataFlow):
    """
...
class RandomFramesBatchFromCacheSre(SreMfccFiles):
    def __init__(self, cache_dir, percent=100, context=50):
        ...
        self.num_batches_in_epoch = int(len(self.batch_x_paths)*percent/100.0)
        ...
        print("Using", percent, "% of training data")

    def get_n_spks(self):
        return len(self.mapping)

    def get_data(self):
        idxs = np.arange(self.num_batches_in_epoch)
        random.shuffle(idxs)
...
class RandomFramesBatchRsr2015(SreMfccFiles):
...
class WholeUtteranceAsFrameRsr2015(SreMfccFiles):
    """
...
class WholeUtteranceAsBatchRsr2015(SreMfccFiles):
    """
    Produces MFCC frames of size [context*mfcc_size], and corresponding
    numeric label based on mapping create from RsrMfccFiles. mfcc_size
    is n_mfccs=20 if not including double deltas otherwise n_mfccs*3
